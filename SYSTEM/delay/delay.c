...
void delay_ms(u16 nms)
{
    ...
    TIM_TimeBaseStructure.TIM_Period = 1000 - 1; // 计数值为1000次（从0到999）
    TIM_TimeBaseStructure.TIM_Prescaler = 72 - 1; // 预分频系数为72（系统时钟为72MHz，分频后为1MHz）
    ...
    for (uint16_t i = 0; i < nms; i++) {
        while (!TIM_GetFlagStatus(TIM6, TIM_FLAG_Update)); // 等待更新事件
        TIM_ClearFlag(TIM6, TIM_FLAG_Update); // 清除更新标志
    }

    TIM_Cmd(TIM6, DISABLE); // 关闭定时器
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM6, DISABLE); // 关闭TIM6时钟
